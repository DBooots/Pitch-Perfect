...
namespace Accord.Math.Optimization
{
    ...
    public sealed class BrentSearch : IOptimizationMethod<double, double>
    {
        ...
        /// <summary>
        ///   Gets the number of iterations found in the last call
        ///   to <see cref="Minimize()"/>, <see cref="Maximize()"/>,
        ///   <see cref="Find(double)"/>, or <see cref="FindRoot()"/>.
        /// </summary>
            ...
            int maxIterations)
        {
            ...
            for (int i = 0; i < maxIterations; i++)
            {
                ...
                if (Math.Abs(x - middle_range) + range / 2 <= 2 * tol_act)
                    return new BrentSearchResult(x, fx, BrentSearchStatus.Success, i);
                ...
                // min and reduce the enveloping range
                {
                    ...
                    if (double.IsNaN(ft) || double.IsInfinity(ft))
                        return new BrentSearchResult(x, fx, BrentSearchStatus.FunctionNotFinite, i);
            ...
            return new BrentSearchResult(x, fx, BrentSearchStatus.MaxIterationsReached, maxIterations);
        }
            ...
            int maxIterations)
        {
            ...
            if (Math.Sign(fa) == Math.Sign(fb))
                return new BrentSearchResult(upperBound, fb, BrentSearchStatus.RootNotBracketed, -1);
            ...
            for (int i = 0; i < maxIterations; i++)
            {
                ...
                if (Math.Abs(new_step) <= tol_act || fb == 0)
                    return new BrentSearchResult(upperBound, fb, BrentSearchStatus.Success, i);
                ...
                if (double.IsNaN(fb) || double.IsInfinity(fb))
                    return new BrentSearchResult(upperBound, fb, BrentSearchStatus.FunctionNotFinite, i);
            ...
            return new BrentSearchResult(upperBound, fb, BrentSearchStatus.MaxIterationsReached, maxIterations);
        }
        ...
        {
            public BrentSearchResult(double solution, double value, BrentSearchStatus status, int iterations)
                : this()
            {
